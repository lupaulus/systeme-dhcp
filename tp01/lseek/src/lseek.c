...
int main(int argc, char** argv)
{
  ...
  fdinput = open(bin_input_param,O_RDONLY); 
  if (fdinput < 0) 
  { 
  ...
  fdoutput = open(bin_output_param, O_WRONLY | O_APPEND | O_CREAT); 
  if (fdoutput < 0) 
  { 
      printf("Cannot open file %s \n", bin_output_param); 
      return EXIT_FAILURE;
  ...
  int filesize = lseek(fdinput, 0, SEEK_END);
  ...
  {    
    filesize--;
    lseek(fdinput, filesize, SEEK_SET);
    nbread = read(fdinput,buff,1);
    nbwrite = write(fdoutput,buff,nbread);
    fprintf(stdout, buff);
